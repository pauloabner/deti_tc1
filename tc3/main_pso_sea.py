...
from utils.abner_math import predict_polynomial, calculate_r2, calculate_sae
    ...
    if data.size > 0:
        ...
        for run in range(NUM_RUNS):
            best_beta, lowest_sse, history = particle_swarm_optimization(
                x, y, SWARM_SIZE, DIMENSION, BOUNDS, MAX_ITERATIONS, error_metric=calculate_sae
            )
            ...
            print(f"Rodada {run+1:02d} concluída | SEA: {lowest_sse:.4e}")
        ...
        print(f"SEA: {best_run['sse']:.4e} | R²: {r2_best:.6f}")
        ...
        for res in top_3_results:
            plt.plot(res['history'], label=f"Rodada {res['run_id']} (SEA={res['sse']:.2e})", linewidth=1.5)
        plt.title('Curvas de Convergência - Otimização PSO (As 3 Melhores Rodadas)')
        plt.xlabel('Iterações')
        plt.ylabel('Soma dos Erros Quadráticos (SEA)')
