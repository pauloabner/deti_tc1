...
from utils.preprocessing import zscore_full, apply_pca_full
import numpy as np
...
def run_pca_experiment():
    ...
    for k in components_list:
        X_pca, var_retained = apply_pca_full(X_norm, num_components=k)
        print(f"\n{'='*70}")
        print(f" EXPERIMENTO COM {k} COMPONENTES PRINCIPAIS (Variância Retida: {var_retained:.2f}%)")
        print(f"{'='*70}")
        ...
        for model_name, model_constructor in models.items():
            ...
            print(f" -> RESULTADOS: {model_name:<7} | {k} Componentes ---")
